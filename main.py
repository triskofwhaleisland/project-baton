...
prefix = '.'
version = "B is for probably Broken"
...
class RecruitQueue:
    ...
    def update_user(self, user: discord.Member, status: RecruitmentStatus | None) -> None:
        if not status:
...
def save_recruiters_to_yaml(queue: dict, filename: str) -> None:
    yaml.safe_dump({user.id: status.name for (user, status) in queue.items()}, open(filename, 'w'))
...
async def set_status(ctx: commands.Context, stat: str):
    try:
        if (
            ...
            await ctx.channel.send(f"{RQ.active_user} is already actively recruiting. Ask them to leave first.")
        else:
            RQ.update_user(ctx.author, RecruitmentStatus[stat.upper()])
            await ctx.channel.send(f"{ctx.author} is now {stat.lower()}.")
    except ValueError:
        await ctx.channel.send(f"{stat.upper()} is not a valid status. "
...
@bot.command()
async def join(ctx: commands.Context): await set_status(ctx, 'active')


@bot.command()
async def ready(ctx: commands.Context): await set_status(ctx, 'ready')
...
async def display(ctx: commands.Context):
    ...
    await ctx.channel.send(embed=discord.Embed(
        ...
        description="Name:\t\t\t\tStatus\n" +
                    (f"**{RQ.active_user}: ACTIVE**\n" if RQ.active_user else "") +
                    "\n".join(f"{user}:\t\t\t\t{status.name}" for
                              (user, status) in RQ.queue.items() if status != RecruitmentStatus.ACTIVE)
